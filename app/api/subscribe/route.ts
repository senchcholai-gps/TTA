...
export async function POST(req: Request) {
  try {
    ...
    const fromSender = process.env.SUBSCRIBER_FROM_EMAIL || process.env.AUDIT_FROM_EMAIL || 'TTA Notifications <onboarding@resend.dev>'
    ...
    if (resend) {
      try {
        await resend.emails.send({
          from: fromSender,
          to: [adminRecipient],
          subject: 'New Website Subscriber — TTA',
          html: `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #eaeaea; border-radius: 16px; background-color: #ffffff;">
              <div style="background: linear-gradient(135deg, #D6003C 0%, #8B0095 50%, #3D00D6 100%); padding: 18px 24px; border-radius: 12px; margin-bottom: 24px;">
                <h2 style="color: #ffffff; margin: 0; font-size: 20px; font-weight: 800; letter-spacing: -0.5px;">New Website Subscriber</h2>
              ...
              <p style="font-size: 15px; color: #333333; margin-bottom: 20px; line-height: 1.6;">
                A new subscriber has submitted their email address through the website.
              </p>
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                  ...
                  <tr>
                    ...
                    <td style="padding: 10px 0;">
                      <span style="background-color: #ede9fe; color: #6d28d9; padding: 4px 10px; border-radius: 9999px; font-size: 12px; font-weight: 700; text-transform: uppercase;">New</span>
                    </td>
                  </tr>
                </table>
              </div>
              ...
              <div style="text-align: center; margin-top: 24px;">
                <a href="mailto:${normalizedEmail}?subject=The%20Three%20Amigos%20-%20Welcome" style="display: inline-block; background-color: #1A1A1A; color: #ffffff; padding: 12px 28px; border-radius: 10px; font-size: 13px; font-weight: 700; text-decoration: none;">
                  Contact Subscriber
                </a>
              </div>
              ...
              <p style="font-size: 11px; color: #999999; text-align: center; margin: 0;">
                This notification was sent automatically to admin when a user subscribed on the website.
              </p>
            </div>
          `,
          text: `
A new subscriber has submitted their email through the website.
