...
export default function CaseStudiesSection() {
  ...
  return (
    ...
    >
      ...
      {/* Thin connecting gradient lines — mimic data-flow */}
      {!shouldReduceMotion && (
        <>
          <motion.div
            aria-hidden="true"
            animate={{ scaleX: [0.4, 1, 0.4], opacity: [0.03, 0.07, 0.03] }}
            transition={{ duration: 14, repeat: Infinity, ease: 'easeInOut' }}
            className="absolute top-[30%] left-0 right-0 h-px bg-gradient-to-r from-transparent via-brand-red to-transparent pointer-events-none -z-10"
          />
          <motion.div
            aria-hidden="true"
            animate={{ scaleX: [1, 0.5, 1], opacity: [0.03, 0.06, 0.03] }}
            transition={{ duration: 18, repeat: Infinity, ease: 'easeInOut', delay: 4 }}
            className="absolute bottom-[35%] left-0 right-0 h-px bg-gradient-to-r from-transparent via-brand-purple to-transparent pointer-events-none -z-10"
          />
          {/* Ambient soft glow — top-left */}
          <motion.div
            aria-hidden="true"
            animate={{ x: [-20, 20, -20], y: [-10, 10, -10], opacity: [0.03, 0.05, 0.03] }}
            transition={{ duration: 20, repeat: Infinity, ease: 'easeInOut' }}
            className="absolute top-0 left-0 w-[400px] h-[400px] bg-gradient-to-br from-brand-red via-brand-magenta to-transparent rounded-full blur-3xl pointer-events-none -z-10"
          />
          ...
          <motion.div
            ...
            className="absolute bottom-0 right-0 w-[400px] h-[400px] bg-gradient-to-tl from-brand-purple via-brand-magenta to-transparent rounded-full blur-3xl pointer-events-none -z-10"
          />
        </>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5 lg:gap-8"
